...
- **iCloud auto-download (one-time setup)**: use **Download iCloud Folder Locally…** once to pick a folder (e.g. your GitHub backup) — it downloads it, then offers **Auto-Download Hourly**: while keep-awake is on, the app re-requests the download every hour by itself, so the folder simply stays fully on this Mac with no further steps. Toggle it off (or pick a different folder) from the same menu. Done = no cloud ☁ icons in Finder.
